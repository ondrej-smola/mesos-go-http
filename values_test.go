...
var _ = Describe("Values", func() {
	...
	It("Set_Compare", func() {
		for i, tc := range []struct {
			...
			if x != tc.want {
				Fail(fmt.Sprintf("test case %d failed: expected %v instead of %v", i, tc.want, x))
			}
			if !preleft.Equal(tc.left) {
				Fail(fmt.Sprintf("test case %d failed: before(left) != after(left): %#+v != %#+v", i, preleft, tc.left))
			}
			if !preright.Equal(tc.right) {
				Fail(fmt.Sprintf("test case %d failed: before(right) != after(right): %#+v != %#+v", i, preright, tc.right))
			}
		}
	})

	It("Set_Subtract", func() {
		for i, tc := range []struct {
	...
	It("Ranges_Add", func() {
		for i, tc := range []struct {
			...
			x := tc.left.Add(tc.right)
			if !x.Equal(tc.want) {
				Fail(fmt.Sprintf("expected %v instead of %v", tc.want, x))
			}
			if !preleft.Equal(tc.left) {
				Fail(fmt.Sprintf("test case %d failed: before(left) != after(left): %#+v != %#+v", i, preleft, tc.left))
			}
