...
var _ = Describe("Client", func() {
	...
	It("Send POST", func() {
		cl := New(endpoint, WithHttpDoFunc(func(r *http.Request) (*http.Response, error) {
	...
	It("Set Context-Type from codec", func() {
		cl := New(endpoint, WithCodec(codec.ProtobufCodec), WithHttpDoFunc(func(r *http.Request) (*http.Response, error) {
	...
	It("Add request header", func() {
		...
		_, err := cl.Do(msg, context.Background(), WithHeader("TEST", "TEST"))
		Expect(err).To(Succeed())
	})
	...
	It("Map errors - 503", func() {
		...
		_, err := cl.Do(msg, context.Background())
		Expect(err).To(Equal(Unavailable))
	})
