...
func Blueprint(client client.Client, opts ...Opt) flow.SinkBlueprint {
	return flow.SinkBlueprintFunc(func(matOpts ...flow.MatOpt) flow.Sink {
		cfg := flow.MatOpts(matOpts).Config()

		if cfg.Log != nil {
			opts = append(opts, WithLogger(log.With(cfg.Log, "src", "scheduler_client")))
		}
...
func New(client client.Client, opts ...Opt) *Client {
...
func (c *Client) doWrite(w chan *event, streamId string) {
	...
	case *Call:
		resp, err := c.client.Do(r, ev.ctx, client.WithMesosStreamId(streamId))
		if err == nil {
			resp.Close()
		} else {
			w <- &event{err: err}
...
func (c *Client) subscribe(ev *event) (client.Response, error) {
	...
	// set Close(true) because subscribe call should not reuse connections
	if res, err := c.client.Do(subscribe, c.ctx, client.WithClose(true)); err != nil {
		return nil, err
	} else {
		sid := res.StreamId()
		if sid == "" {
			return nil, errors.Errorf("Mesos is expected to set %v but it is empty", client.MESOS_STREAM_ID_HEADER)
		}
		c.streamId = sid
		return res, nil
	}
...
func (c *Client) readerLoop(resp client.Response) {
